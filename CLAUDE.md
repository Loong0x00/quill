...
- **部署(实测路径)**:web 资产 `include_str!` 烤进 **`quill-kernel`** → 改 `index.html` 必须**重建 + 重装 quill-kernel**(`cargo build --release --bin quill --bin quill-kernel`,`install -m 755` 原子免 ETXTBSY)。**✅ 两套安装位是【有意的稳定/测试分流】(2026-06-30 用户定,非待收口)**:
  ...
  - **📍 当前安装快照(2026-07-02,两处已【合流】)**:`~/.local` 与 `/usr/local`(稳定日用/图标)**同源** = 最新(砖2 + G1 联邦 + F4 分段栏 + F4+ 每窗口新建 + 关窗口最后一个 tab,`quill dc14bd…` / `quill-kernel cb9b8e…`)。用户 2026-07-02 实测本机联邦端到端可用后拍板 `sudo install` 提升到 `/usr/local`(图标日用也吃上;日用不 --share、联邦/共享全是 Ctrl+Shift+S opt-in,无影响)。soak 期结束、分叉合流。
...
- **★ 北极星方向:所有 quill 框进一个 kernel(本机联邦)** —— 每台一个 quill-kernel **单例**(独占 7878),各 quill 窗口**不再各自 spawn kernel、而是会合(本地 socket/锁)接入同一个**当 feeder,**每个窗口 = 一个 workspace**,手机看**全部窗口的 tab 聚合视图**。地基现成(砖0 多 workspace + feed (ws,tab) 标签 + 砖2 每客户端独立看)。要新造:① 会合(我是第一个?→拉 kernel 占锁;否则接入)② **kernel 生命周期从"某窗口的子"改成"机器级单例、detach、feeder refcount 归零才关"**(修订 E′,值 ADR)。= 早先 LAN 联邦愿景的**本机版**,顺带彻底消除 7878 撞车。用户提(2026-07-01)。**★ ADR 已定稿 = [ADR-0019](docs/adr/0019-federation-single-kernel-share-anchor.md)**:联邦(超集)+ 会合(本地 socket:找已有→挂/建)+ kernel 机器级单例/detach/feeder refcount + **home 锚 = 用户开共享的那个窗口(= 有语义的 #0,新 tab 落这里)** + 按设备分段(窗口是设备本地细节)+ 分砖 F1–F5。
  - **★ [ADR-0020](docs/adr/0020-kernel-boot-service-lockfree-rendezvous.md)(修订/延伸 0019,也是修联邦 #3 的 spec)**:① **无锁会合** —— 删 flock(竞态实际不可达:共享手动 opt-in、人手做不到微秒双开、现实脚本不产生;真撞 bind 失败秒退自愈),退化成 **try-connect-else-spawn + 连接 calloop 异步(永不阻塞终端)** → **#3 消失**;② **kernel 升开机无头服务**(systemd user service,无需 GUI/人在场即 serve;GUI 窗口=可选 feeder;kernel 可拥有远程无本地窗口的会话);③ **★终端永远优先(重申 E′)** —— quill 先起终端+开 tab,连服务是【之后、额外、非阻塞】一步,服务挂/慢/无都不碰终端。分砖 G1(=修 #3:无锁异步会合)→ G2(kernel 服务化)→ G3(kernel 远程会话)。
  - ✅ **砖 G1(=修联邦 #3,本机联邦可用)** `eb4890a`..`938bc47`(2026-07-02):**多窗口经无锁 calloop 异步会合接入机器级单例 kernel**。kernel 侧多 feeder 汇入多 workspace + refcount 生命周期(`eb4890a`)+ F3 手机新 tab 落共享锚窗口(`f722af7`);**修 daemon.rs fd 双关**(`make_feeder` 事务化:READ 源注册后任一步失败必 `loop_handle.remove` 回滚、`read_fd==write_fd` dup 写端避免双 OwnedFd 双关同一 fd、`attach_feeder` Err 臂删 `libc::close(raw)`,`28ddce6`)+ **会合无锁异步**(window.rs feeder-over-socket ShareChild:`start_share`→**非阻塞 try-connect**→`Ok(Some)`即接入 / `Ok(None)`拉 detached kernel + **calloop Timer 异步重试**;删旧 flock 三函数;3s deadline→琥珀,`27d31f1`)+ **终端优先收口**(`share_spawn_detached_kernel` 不再同步 `child.wait()`——那会冻终端到 fork1 原进程 `_exit`、kernel 早期卡死更永久冻;改 retry tick 非阻塞 `try_wait` 收尸,`938bc47`)。#3(flock 多秒冻结)+ fd 双关**都真修好**;工作流 impl(opus xhigh)+ 3 路对抗审 + synth 独立复跑 CI;synth 逮出的同步 wait 由 Lead 收口。CI 全绿(lib **546** + kernel_federation_slice **4** + tab e2e 全过)。**合 main + 装 ~/.local;/usr/local 稳定版仍 soak(pre-砖2)**。旧 `feat/federation`(flock 同步会合版)留作参照未删。
  - **下一步 = 砖 G2**(kernel 能独立零-feeder 启动运行 + systemd user service 单元 + 开机自启,ADR-0020 §1)→ 之后 G3(kernel 拥有的远程无本地窗口会话)。
- ✅ **tab 拖拽动画** `3664f8b`..`9aedd5e`(2026-07-02):浏览器式 tab 换位 —— 拖起 tab 跟手抬起(阴影+略亮)、其它 tab **邻接 swap 累积让位**(补间非 snap、复用 swap_reorder 没改重排语义)、松手从光标缓动落位。**新建可复用动画基建**:`TabAnimState`(按 tab **id** 存视觉 x、扛 live reorder)+ 纯函数 `tab_anim_ease`(帧率无关指数缓动 TAU=30ms,~150–170ms 落位)+ calloop Timer 逐帧,**settle 即 `TimeoutAction::Drop` + token=None → 空闲零成本**(同 repeat/autoscroll 套路,无 60fps 常驻)。碰 pointer/render/window.rs;**render 仅 tab 栏支路、headless None 逐像素同旧、默认零回归**;3 路对抗审 APPROVE,CI lib 545。装 ~/.local(动画在 GUI 侧、kernel 不变)。
- ✅ **砖 F4(联邦 web 分段标签栏 + 跨窗口切 tab)** `2ff279f`..`bb9a48a`(2026-07-02):联邦(G1)后浏览器只见一个窗口的 tab —— 根因①连上引导 + 广播只发【attach 窗口】的 `Workspace` 明细(别窗口只有 `Workspaces` 里的 tab_count,无 tabs)②`TabOp::Select` 忽略消息 `workspace_id`、只在 attach feeder 内切。**kernel**(daemon.rs,只碰联邦路径、`src/wl/*` 一行没动):引导 `build_control_text_feeder` + `broadcast_feeder_metadata` 改发**全部 feeder** 的 `Workspace` 给**所有** live 客户端(`2ff279f`)+ `handle_tab_op` Select/Close/Reorder 按消息 `workspace_id` 经 `feeder_id_for_ws` **跨窗口路由** + Select 更 `c.feeder_id` 到目标窗口(New 仍落锚 F3)。**web**(index.html):`Map<ws,{tabs,active}>` 状态 + **按窗口分段渲染**(单窗口不画段头=不退化)+ viewed tab 跨段唯一高亮 + 每窗口桌面焦点点 + 点跨窗口 tab 发 `TabOp{workspace_id,Select}` + termInstances 改复合键 `viewKey(ws,tab)`(修跨窗 tab_id 撞键)。**Lead 收口**:F4 让 `feeder_id` 可与 `viewed` 分叉 → New 自动 Select(`feeder_tab_list_updated` pending 块)漏对齐 feeder_id(跨窗口 Select 后点 + 落锚 → dims 广播/orphan 回收漏本客户端),补对齐 + 回归测(以锚 Dims 送达为可观测判据,去修复即失败,`bb9a48a`)。工作流 impl(opus xhigh)+ 3 路对抗审 + synth;2 视角独立命中分叉 bug、synth CONFIRMED。CI 全绿(lib **546** + kernel_federation_slice **8** + tab e2e 全过;web 无自动化 harness,靠 kernel 测 + 手测)。**合 main + 装 ~/.local(F4 web+路由烤进 quill-kernel;quill 窗口二进制 sha 不变=F4 全在 kernel 侧,窗口无需重启)。**
  - ✅ **F4+ 每窗口新建 tab** `82b065a`(2026-07-02,用户提):分段栏既已能"指窗口"(ADR-0019 §4 显式表达),New 不必再一律落锚 —— kernel `handle_tab_op` New 改 `feeder_id_for_ws(workspace_id).or(anchor).or(attach)`(落所点窗口,找不到回落锚=F3 默认 home);web 把 `+` 从全局一个移进**每个窗口段各一个**(点谁的 + 落谁),单窗口时仍是唯一 +。Close/Reorder F4 已按 workspace_id 路由,New 至此对齐。锚仍是 home 语义(回落)。+1 kernel 测(点非锚窗口 + → New 落该窗口、锚不收);CI lib 546 / federation slice 9 / tab e2e 全过。改小(daemon 13 行 + web),Lead 直接做+测(未开 workflow)。装 ~/.local。
  - ✅ **手机可关窗口最后一个 tab** `bbf53b1`(2026-07-02,用户提):去掉旧护栏"远程 Close 不许关最后一个 tab"。关最后一个 = `close_tab_idx` 关到空 → `loop_signal.stop()` 让【该窗口进程】优雅退出(每窗口独立进程,只退它自己;联邦按 feeder 断线回收该 workspace),与桌面 Ctrl+W 对称;web 每个 tab 都渲 ×(不再 ≥2 才渲)。改动在 **window.rs(route_share_tab_op)+ web** → ⚠️**quill 窗口二进制变了**(与 F4/F4+ 仅 kernel 变不同),**须重启窗口**才生效(不是光换 kernel)。默认无共享路径零回归;CI lib 546 / tab e2e 全过。
- 🔒 **全代码库安全审计(2026-07-02)** `docs/notes/security-audit-2026-07-02.md`(**本地未提交,别推 public repo=未修 RCE 路线图**):17 路 finder + 对抗验证,19 条去重根因。★结论=代码无内存/加密灾难,真问题是架构(kernel 无 auth 裸绑 0.0.0.0:7878)。**CRITICAL=CSWSH**(WS 握手无 Origin/token→恶意网页借浏览器连本机 7878=RCE+偷全终端,绕 VPN)。详见 [[project_quill_security_audit_2026-07-02]]。
- ✅ **P0 CSWSH 修复(堵唯一 CRITICAL)** `4bc1e1a`..`03961cd`(2026-07-02):WS 握手 `NoCallback`→`AuthCallback` 强制**token**(32B CSPRNG getrandom→64hex,常量时间比;持久 `~/.config/quill/token` 0600,经 `QUILL_SHARE_TOKEN` env 传 spawn 的 kernel,放分享 URL `?t=`,恶意网页拿不到即 403)+ **Origin/Host 校验**(纵深,Host 防 DNS rebinding;放行任意 IP-literal+localhost+env `QUILL_SHARE_ALLOWED_HOSTS`,不误杀手机 LAN/VPN IP)+ **max_message_size 1MiB**(灭审计 #2/#14 单条大消息杀 kernel)。**不绑 loopback**(手机要连,安全靠 token)。web 从 `?t=` 读 token 连 WS + `sendBytes` ≤64KiB 分帧(大粘贴照发、单帧不超限)。getrandom 提直接依赖。工作流 impl+3 审+synth;synth 逮出 impl **漏更 kernel_feed_slice**(假绿:CI 链没含它)→ Lead 补 token。**顺带修 F4 回归**(`218b256`):Close/Reorder 按 workspace_id 找不到 feeder 无回退(F4 改了却漏跑 feed_slice)→ 加 `.or(client feeder_id)` 回退(**不用 anchor** 免关错窗口);feed_slice 8/8 恢复。CI 真绿(lib **558** + ws 14 + federation 9 + feed 8 + tab e2e)。**合 main + 装 ~/.local;/usr/local 仍旧版(CSWSH 只在开共享时暴露),待用户测 token UX 再提**。ECC/RSA 是过度设计(单用户,token 够)。P1 backlog(资源上限/bracketed-paste 注入等)见审计报告。⚠️分享 URL 现打 journald,含 token;手机存带 `?t=` 的完整 URL 一次即可。
- ✅ **窗口控制图标改矢量 AA 线段(× / − / □ 构造一致)** `171c86a`(2026-07-02,用户提:关闭 × "发糊发虚没力量"):根因=close × 走**字体字形**(U+00D7 经 cosmic-text glyph atlas,又细又淡)而 min/max 走**轴对齐 rounded-quad**——两套机制不一致。按标准桌面 UI 做法(矢量图标、分辨率无关、主题色)**新建独立「抗锯齿矢量线段 stroke」图元**(`LINE_WGSL` 平头盒 SDF + `create_line_pipeline` 无 bind group、与共用 rounded shader 隔离 + `append_line_seg_px`/`append_line_rect_px`),**× / − / □ / + / share 全走它**、同 `stroke_w=2×hidpi`、同 `icon_color`。close ×=2 真-AA 斜线(硬、亮、同粗,取代软字形);min/max 轴对齐线段**整像素吸附**→ SDF 塌成硬边,**逐像素 AE=0 不退化**(独立 magick compare 证实)。两渲染路径都渲 + **headless 首次渲出 close ×**(补测试盲区)+ 删旧 `append_close_icon_glyph`。只碰 `render.rs`(+465/-175)、pointer/window.rs 零行、hit-test 未变、单线程零回归。工作流 impl+2 审+synth **approve**;CI lib 558 + tab e2e 全绿。**⚠️ 改的是 render.rs=quill 窗口二进制变了,须重启窗口生效**。装 ~/.local。用户看放大图确认"有力量"。**可复用**:LSP 无头出图(`quill --headless-screenshot=`)+ imagemagick crop 放大 = AI 能亲眼判视觉 bug(不用只靠用户描述)。
  - ✅ **窗口控制图标像素级标定** `a1f6b92`..`d2c6d9c`(2026-07-02,用户真机 6K 逐值拨定):在矢量线段图元上把 min/max/×/− 调到最平衡 —— `ICON_STROKE_L=1.5`(min/max/− 线宽逻辑px)、`X_STROKE_L=2.1`(× 单独加粗)、`X_EXTRA_INSET_L=0.95`(× 比 □ 每边多缩,× 铺满对角线视觉比同框 □ 大)、`X_YBOT_PX=1`(× 下边下扩,光学重心:×与□几何中心重合但交叉点vs横边视觉重量分布不同显错位)。**★别再为图标锐度造 SSAA/超采样轮子**:× 糊是斜线光栅固有 —— **直线的解析 SDF AA(现用的 `clamp(0.5-d)`)= 无限超采样的极限,SSAA 对干净直线零增益**(只对曲线/文字/锯齿有用),且全屏 SSAA 会糊终端文字 + 6K 上重。唯一缓解=把 × 加粗让实心核盖过 ~1px 灰边。用 `--headless-screenshot` 渲图 + imagemagick **最近邻**(`-filter point`,别用会糊的 `-scale`)放大 + PIL 量重心/边宽 = AI 能亲眼闭环判视觉。曾一度做 env 旋钮(QUILL_ICON_STROKE 等)让用户真机拨、拨定后烤成常量删脚手架。
- **T7** web"X 关闭"语义 / PWA 壳 → **硬化**(真网格关键帧替字节尾、bincode、带宽)+ 审计 P1 资源上限批。
**⚠️ 凡碰 render/wl 的 ticket(砖1 起),impl/审码 agent 必须自己逐行读 `wl/render.rs`、`wl/window.rs` draw 路径,别信摘要。**
**⏱️ CI:`ci.sh --fast`(fmt+clippy+build)≈17s 增量;全门含 GPU/wl e2e(需本机 GPU+Wayland、串行)较慢(十几分钟级)。lib 现 ~546 测;kernel 侧 slice(`kernel_ws_slice`/`kernel_feed_slice`/`kernel_daemon_slice`)+ wl e2e(`single_tab_no_bar`/`multi_tab`/`titlebar_text`/`csd`/`ghostty_tab_polish`…)。**kernel 改动与 render/wl 改动互不影响对方那批** → 可只跑快门 + 相关子集(kernel/feed 改 → 跑 kernel slices;render/window 改 → 跑 titlebar/csd/single_tab)。⚠️ `kernel_ws_slice` 含 slowloris/背压【计时】测,单跑 ~1–2min、高负载下偶 flaky(与业务改动无关,单跑复现即过)。**
...
# 构建 + 四门验收
cargo build
cargo test                            # lib ~546 测 + kernel/feed/ws slices + GPU/wl e2e(见上 Phase 7 CI 行)
