...
- **砖1b(单作者,碰 render/wl)**:父监管共享子进程 + **tee PTY 输出喂子(tee 点 window.rs:568,转原始 `buf[..n]` 非 term_bytes,只焦点 tab)** + 子→父 back-channel 读源接 `queue_or_write_pty` + 桌面接入。`share: Option<ShareChild>` 挂 `LoopData`、像 `selection_state` 那样 `Option` 穿进 `pty_read_tick(_inner)`;**`render.rs`/键鼠/selection 零触点 → render.rs 不改**;`main.rs` 加缺省 false 的 `--share` 解析(opt-in 武装即起,见 ADR-0018);INV-001/008 drop 序须 cargo 编译实证。
