        ...
        .run(None, &mut loop_data, |data| {
            ...
            let draw_result = match text_system.as_mut() {
                Some(ts) => {
                    ...
                    r.draw_frame(ts, &cells, cols, rows, &row_texts, preedit_overlay.as_ref())
                }
            ...
            // T-0505: cursor_rectangle 上报 (派单 In #E). cursor 位置每帧
            // 都可能变 (PTY 输出 / 用户敲方向键), 这里读 term.cursor_pos()
            // 算 logical px → ime_state.update_cursor_rectangle 检测变化 →
            // 仅当变化时调 text_input.set_cursor_rectangle + commit (减
            // protocol round-trip)。仅当 IME enabled (focus on quill) 时上
            // 报, 否则浪费 (fcitx5 候选框已 hide)。
...
struct State {
    ...
    // T-0505: zwp_text_input_v3 (TIv3) IME 协议绑定。
    //
    // `text_input_manager`: registry bind 一次, GlobalProxy 模式 (compositor
    // 不导出 zwp_text_input_manager_v3 时 None — fcitx5 / ibus 未启 / 旧版
    // mutter 等); ime_state 仍 work, 只是接不到 IME 事件。
    // `text_input`: 在 Keyboard capability 出现 + 已有 manager 时一并 get,
    // 与 wl_keyboard 同生命周期 (二者都跟 seat keyboard capability 走)。
    // `ime_state`: 业务状态 (preedit / pending / cursor_rect 缓存), quill 自
    // 有 (INV-010 类型隔离 — wayland-protocols 类型不出 ImeState 公共 API)。
    //
    // drop 顺序: text_input → text_input_manager → ime_state, 都不持 wgpu
    // 裸指针, 放此处与 keyboard / pty 同生命周期带, 不破 INV-001。
    // wayland-protocols 协议对象 drop 时发 destroy request, compositor 接收
    // 后释放 server 端资源, 与 wl_keyboard.release() 同套路。
...
impl SeatHandler for State {
    ...
    fn remove_seat(&mut self, _: &Connection, _: &QueueHandle<Self>, _: wl_seat::WlSeat) {
        // seat 整个移除 — 当前 wl_keyboard / text_input 一并失效, 走 take 释放。
        if let Some(kb) = self.keyboard.take() {
            kb.release();
        }
        // T-0505: 同 remove_capability(Keyboard) 路径。
