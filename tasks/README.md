...
任务队列目录。Implementer teammate 在此抢未认领的 ticket,领取后在自己的 worktree 干活。
...
1. Implementer 扫描 `tasks/` 找 `Status: open` 的 ticket
...
4. 完成后 `Status` 改为 `in-review`,commit 到 feature 分支并通知 Gatekeeper
5. Gatekeeper 放行 → Implementer 合并到 main → `Status` 改为 `merged`
