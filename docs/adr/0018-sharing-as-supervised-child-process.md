...
- **共享 = 懒启动、受父监管的【子进程】**:
  - **没手机连入 → 根本不 spawn 子进程**(无管子、零开销,终端就是今天的终端);首个手机连入才拉起,全断了收掉。
  - 子进程 = ADR-0017 那套**单线程 calloop WS-kernel**(WS 监听 + 同口 HTTP + 客户端 + reaper),但**字节从父的 tee 来**,不自 spawn shell。
  - 子进程崩溃(panic/abort/UB/OOM)= **OS 级隔绝**,父最多丢掉 / 重启共享,**终端无感**。
