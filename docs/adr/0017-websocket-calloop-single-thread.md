...
- **listener**:`TcpListener` 注册成 owned `Generic` READ 源;可读→`accept` 排空到
  ...
  信号 + `peek` 句柄)。
- **同口 HTTP / WS 分流**:连接 READ 源首次可读时 `peek`(MSG_PEEK 非破坏)出请求头;头
  没收全就 `Continue` 等下次可读(**不 `thread::sleep`**)。带 `Upgrade: websocket` 走 WS,
  否则当 HTTP(响应排进一个非阻塞写源写完即 `Remove`)。
...
- **半截连接超时**:发了部分 HTTP 头就不发 / 握手卡住的连接会占一个 `MAX_WS_CONNS` 槽,
  靠对端关闭或 16 上限兜底,无主动超时收割(硬化)。
- **探活**:去掉了线程版的周期 Ping;靠 READ 源 EOF + 出站 cap 回收死连接,silent half-open
  idle 连接需 TCP keepalive(未调 idle,硬化)。

## 实装验证

- `src/kernel/daemon.rs` 全 calloop 化(无 `std::thread` / `mpsc` / `Arc` / `Mutex`)。
- `tests/kernel_ws_slice.rs` 4 条端到端(真子进程):同口 HTTP 出页、WS 字节流重放+live
