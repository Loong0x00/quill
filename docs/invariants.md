...
rounded_pipeline       (Option<wgpu::RenderPipeline>, T-0615+, 持 device 引用, per-vertex elem_bounds + elem_radius rounded SDF, 40 字节顶点)
rounded_vertex_buffer  (Option<wgpu::Buffer>, T-0615+, 持 device 引用)
rounded_buffer_capacity (usize, T-0615+, 仅元数据无 GPU 引用, 顺序无关但放此处保 visual locality, ~50 quads / frame)
corner_uniform_buffer  (wgpu::Buffer, T-0610 part 2+, 持 device 引用, std140 [w,h,r,alpha_live] 16 字节)
corner_bind_group      (wgpu::BindGroup, T-0610 part 2+, 持 device + uniform Arc, group=1 共享 cell+glyph+rounded pipeline)
device                 (持 GPU context, 必须在 surface + cell/glyph/rounded/corner 资源后 drop)
...
title                  (String POD, T-0702+, CSD titlebar 中央标题文字, 顺序无关)
tab_count              (usize POD, T-0608+, 标签条 UI 渲染当前 tab 总数, 顺序无关)
active_tab_idx         (usize POD, T-0608+, 标签条 UI 渲染 active tab 高亮索引, 顺序无关)
alpha_live             (f32 POD, T-0610 part 2+, live wayland 0.85 / Opaque/headless 1.0, resize 路径复用)
instance               ←  最后 drop (Vulkan/GL 底层实例)
```

**为啥**:`wgpu::Surface` 依赖 `wgpu::Instance` 保持 Vulkan/GL 底层实例存活。surface 必须在 instance 之前 drop。任何持 device 内部引用的 GPU 资源 (`RenderPipeline` / `Buffer` / `Texture` / `TextureView` / `Sampler` / `BindGroup` / `BindGroupLayout`) 必须在 device drop 之前 drop —— T-0305 起 `cell_pipeline` / `cell_vertex_buffer` 加, T-0403 起 `glyph_atlas` (含全套 GPU 资源) / `glyph_pipeline` / `glyph_vertex_buffer` 加, T-0615 起 `rounded_pipeline` / `rounded_vertex_buffer` 加 (per-vertex elem_bounds + elem_radius rounded SDF pipeline)。device / queue 依赖 adapter (已构造后立即 drop, 但 device 内部引用保持 GPU context 存活), 所以 device 在 queue 前或后都行, 但必须在 surface + 全部 GPU 资源之后、instance 之前。POD 字段 (cell_buffer_capacity / glyph_buffer_capacity / rounded_buffer_capacity / config / clear / surface_is_srgb) drop 顺序无关, 但保持声明顺序 visual locality 让 review 时易查。
...
- T-0702 加 title (15 字段, CSD titlebar 中央标题文字 String POD)
- T-0608 加 tab_count + active_tab_idx (17 字段, 标签条 UI 渲染参数, usize POD 顺序无关)
- **T-0610 part 2 加 corner_uniform_buffer + corner_bind_group + alpha_live (20 字段, 圆角 + 半透明 alpha=0.85, corner_* 持 device 引用必须在 device 之前 — reviewer-T0610 否决 hotfix d1807ea 修字段顺序; alpha_live POD 顺序无关)**
- **T-0615 加 rounded_pipeline + rounded_vertex_buffer + rounded_buffer_capacity (23 字段, ghostty / GTK4 风 tab UI polish — per-vertex elem_bounds + elem_radius SDF pipeline 给圆角 tab body / + 按钮 box / 圆形 window button 用; rounded_pipeline + rounded_vertex_buffer 持 device 引用必须在 device 之前; rounded_buffer_capacity POD 顺序无关; per-vertex 取代 派单原 uniform array 设计避免 bg fill quad 圆角处 discard 露透明 bug)**
