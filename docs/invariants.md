...
```
surface              ←  先 drop (第 1)
cell_pipeline        (Option<wgpu::RenderPipeline>, T-0305+)
cell_vertex_buffer   (Option<wgpu::Buffer>, T-0305+)
device
queue
config
clear
...
**为啥**:`wgpu::Surface` 依赖 `wgpu::Instance` 保持 Vulkan/GL 底层实例存活。surface 必须在 instance 之前 drop。`RenderPipeline` / `Buffer` (T-0305 cell 渲染加) 持 device 引用, 必须在 device drop 之前 drop。device / queue 依赖 adapter(已构造后立即 drop,但 device 内部引用保持 GPU context 存活),所以 device 在 queue 前或后都行,但必须在 surface + cell_pipeline + cell_vertex_buffer 之后、instance 之前。
