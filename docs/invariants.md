...
**约束**:字段声明顺序必须是 (20 字段, T-0610 part 2 扩展)
```
...
corner_bind_group      (wgpu::BindGroup, T-0610 part 2+, 持 device + uniform Arc, group=1 共享 cell+glyph pipeline)
device                 (持 GPU context, 必须在 surface + cell/glyph/corner 资源后 drop)
