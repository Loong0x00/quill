...
config
clear
instance   ←  最后 drop (第 6)
```

**为啥**:`wgpu::Surface` 依赖 `wgpu::Instance` 保持 Vulkan/GL 底层实例存活。surface 必须在 instance 之前 drop。device / queue 依赖 adapter(已构造后立即 drop,但 device 内部引用保持 GPU context 存活),所以 device 在 queue 前或后都行,但必须在 surface 后、instance 前。
