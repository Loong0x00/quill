...
## Phase 8 — polish++ (terminal correctness + daily-drive feel, 2026-04-26~05-02)

**已完工 ticket** (按时间):
- [x] `T-0801` CJK 字形 forced 双宽 advance (cosmic-text 自然 1.67× → 严 2× 居中) ✅ merged 2026-04-26
- [x] `T-0802` resize 节流 + wgpu present_mode Mailbox (修拖窗口卡顿延迟) ✅ merged 2026-04-26
- [x] `T-0608 hotfix` pixel_to_cell 多 tab top_reserved 修 row 错位 1 (selection/pointer 接口) ✅ merged 2026-05-02 (446f4fa)
- [x] `T-0803` switch grid width judgment to unicode-width crate (PUA / nerd font 跟 lsd 对齐, ADR 0010) ✅ merged 2026-05-02 (afc8d63 + a4fe710)
- [x] `T-0804` selection coords switch to scrollback-absolute SelectionPos (修拖拽蓝框跟字滚, ADR 0011) ✅ merged 2026-05-02 (231ce8a + a778100)
- [x] `T-0805` selection copy supports cross-scrollback (复用既有 scrollback_line_text, 跨 history+viewport 真正复制完整内容) ✅ merged 2026-05-02 (6caac32)
- [x] `T-0806` key dispatch preedit guard (preedit 非空吞 key 不送 PTY, 防 IME 状态混乱时 backspace destructive 删 PTY 已 commit 字符) ✅ merged 2026-05-02 (e021a75)
- [x] `T-0807` Unicode 宽度 / spacer 协议收口 (M1+M2+m1+n1, codex review follow-up): force_cjk_double_advance 按 cluster 聚合 (零宽 / ZWJ / variation selector / 共享 cluster 不再多算 cell) + scrollback_line_text_with_spacers 镜像 viewport 协议 (跨 scrollback 复制 CJK 不夹空格) + preedit underline 宽度按 unicode-width 不按 char count + extract_selection_text 文档同步 e12f276 hotfix 协议 ✅ merged 2026-05-02 (356fb67). audit 抓出 next_cluster_start reverse 扫漏算 multi-glyph cluster bug, hotfix d908d75 转正向 single-pass + 加 ZWJ + ASCII / 三 cluster 测试锁回归
- [x] `T-0809` selection 钉字 / rebase on grid scroll (ADR 0012, 修三 bug 同源): user 实测 yes 流式 / pacman ANSI 重绘 / Claude Code 后台输出 + history view 三场景 selection 框跟 cell 不跟字. 真因 alacritty grid::scroll_up 在 display_offset != 0 时 display_offset += positions auto-pin (源码独立 grep 验证), quill 不调 selection rebase 致渲染公式 viewport_line = sel_line + display_offset 漂移. 修法 pty_read_tick 包装 advance_bytes per-chunk 比 scrollback_size delta 后调 SelectionState::rebase_for_grid_scroll(delta) (anchor/cursor.line -= delta), 渲染公式 (sel - delta) + (D + delta) = sel + D 抵消, 框钉死. 9 测试 (含 alacritty_auto_pins_display_offset_when_history_grows 锁上游行为, 防 alacritty 升级 D-1 假设破) ✅ merged 2026-05-02 (2edbdfd)

**里程碑达成**:`exec quill` 进 login shell daily drive 已稳. 选区跨 scrollback 拖拽 + 复制完整, lsd nerd font 跟 viewport 对齐, IME 异常不会 destructive 删字符, **selection 钉字而非钉 cell 三场景全消** (流式输出 / ANSI 重绘 / history view + 后台 PTY 输出), CJK / ZWJ emoji / combining mark / variation selector 宽度收口跟 unicode-width 严对齐.

**待跟进** (非阻塞):
- pre-existing fmt 漂移 5 处 (render.rs/window.rs/pointer.rs), 后续单 commit 一次性扫掉, 不是回归
- `T-0808` preedit + key-repeat 状态机边界 (codex review M3 "疑虑"级, 触发条件刁钻, ticket spec 已写 backlog)
- 极端 edge case: history 满 (撞 100k 上限) + display_offset > 0 + PTY 持续输出 → alacritty increase_scroll_limit clamp → quill scroll_delta=0 不调 rebase 但 alacritty 仍 display_offset += positions → 框漂. 触发难 (累计 100k+ 行 history view), 决定不开单独 ticket
- IME preedit 视觉跨 viewport 时锚点重定位 (T-0703 follow-up, 低优先级)
...
- 2026-04-27 **T-0618 follow-up part 6-9 daily-drive 修一波 (Lead 直修, claudecode/IME daily-drive 阻塞 9 修)**: 跨 5 文件 (term/pty/wl/render/wl/pointer/wl/window) ~600 行. 用户 daily-drive 实测撞坑驱动: (a) **颜色三连**: COLORTERM=truecolor (claude/Node.js supports-color 走真彩色不退化 256-color) + Tango palette (alacritty 默认 #ef2929 真红替代 xterm-classic 偏粉) + INVERSE flag 解析 (claudecode SGR 7 反相画 cursor cell, 之前 quill 完全忽略致 cursor 不可见 — binary 实测 55 处使用). (b) **per-vertex alpha** (vertex format 5 → 6 f32): bg fill quad alpha=0.85 (壁纸透出), SGR 染色 / cursor / selection / preedit underline alpha=1.0 (满深度). 之前 alpha_live 应用所有 cell quads 致染色 bg 跟壁纸混色色深不对 (claudecode diff 绿/红 bg 浅得难看). (c) **空格 cell bg 修**: build_vertex_bytes 跳过条件 Bg pass 按 cell.bg == DEFAULT_BG, Fg pass 按 cell.c == ' '. 之前先跳所有空格致 SGR + trailing spaces 染整行 bg cell 被忽略. (d) **滚轮 alt-screen 转 cursor 键**: claudecode/vim/less 自处理 transcript, smooth axis 累积器 fallback (mutter 50.x 不发 AxisValue120). (e) **IME preedit 双修**: anchor 在 IME UpdatePreedit 事件 capture (而非 render frame 撞 claudecode 重画中间帧), preedit 变化置 presentation_dirty 触发 redraw (compose 期间 PTY 静默 term not dirty 致 preedit 永不画). 加 titlebar_y_offset_px 让 preedit 不画到 cell grid 上方. (f) **CJK 选区 cell→char idx 错位**: 加 display_text_with_spacers 让 char 数 == cell 数 (spacer 用 \0 占位), 提取后 strip. (g) **PTY back-pressure 队列**: paste 大文件 pty.write 部分写时剩余字节排队 drain_pending_pty_writes 每 tick 试 drain (之前丢字节致 bracketed paste 结束标记没送达 → claudecode 永卡 paste 模式吞键盘). (h) **paste 串行化**: paste_in_progress flag + pending_paste_followups, 防 mutter 同 offer 多次 receive 行为不稳. ⚠️ 大文件连按 Ctrl+Shift+V 仍混乱 deferred (need ghostty clipboard cache). (i) **全屏 / 最大化禁 resize edge**: WindowConfigure.is_fullscreen / is_maximized → set_disable_resize, hit_test 跳 ResizeEdge 退化 TitleBar/TextArea (cursor 是 Default/IBeam 不双向箭头). 9 ticket 完工 (#126/#127/#128/#129/#130/#131/#133/#134/#135/#136), 2 deferred (#132 鼠标 hit-test 偶尔行偏移 不常态难复现 / #137 paste 连按混乱 KNOWN BUG)
- 2026-05-11 **Phase 9 inline composer 规划**: ADR 0013 Accepted。inline composer + popup 候选不依赖 readline。三层降级数据源 (确定性 parser + LLM 补强 + negative cache)。dynamic hooks ~10 个特例 (git/cd/ssh/kill/docker/kubectl/pacman/yay/systemctl)。渲染走 FrameOverlays 重构复用 PreeditOverlay 模式。T1-T11 共 11 ticket,预计增 3,200-4,200 行。
- (后续每个阶段起止在这追加)
