...
- 2026-04-26 **T-0615 ghostty / GTK4 风 tab UI polish 派单中**: User 实测 ghostty 截图对比, quill 当前 tab 是简化版 (矩形 quad / 直角 / 裸 icon). 4 polish: + 按钮包圆角 box / Active tab 圆角矩形 / Inactive tab 透明无 box / Min/Max/Close 圆形 button. 实装路径 wgpu corner mask shader 扩 per-element uniform 数组 (替代当前仅 surface 4 角)
- (后续每个阶段起止在这追加)
